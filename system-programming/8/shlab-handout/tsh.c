...
void eval(char *cmdline) 
{
	...
	sigaddset(&mask, SIGCHLD);//시그널추가
	sigaddset(&mask, SIGTSTP); //trace 09
	sigaddset(&mask, SIGINT);//시그널추가
...
void sigchld_handler(int sig) 
{
	...
	while( (pid = waitpid(-1, &child_status, WNOHANG|WUNTRACED)) > 0){//자식프로세스 종료를 대기
		...
		if(WIFSIGNALED(child_status)){ //자식프로세스 종료를 체크
...
{
	pid_t pid;
	pid = fgpid(jobs); //포어그라운드 프로세스의 pid저장
	 if(pid != 0){
